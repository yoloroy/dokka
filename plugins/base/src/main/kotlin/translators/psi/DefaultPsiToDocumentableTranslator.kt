...
class DefaultPsiToDocumentableTranslator(
    ...
    override suspend fun invoke(sourceSet: DokkaSourceSet, context: DokkaContext): DModule {
        return coroutineScope {
            ...
            DModule(
                ...
                psiFiles.parallelMapNotNull { it.safeAs<PsiJavaFile>() }.groupBy { it.packageName }.asIterable().parallelMap { (packageName, psiFiles) ->
                    ...
                    coroutineScope {
                        DPackage(
                            ...
                            psiFiles.parallelMap { psiFile ->
                                coroutineScope {
                                    psiFile.classes.asIterable().parallelMap { docParser.parseClasslike(it, dri) }
                                }
                            }.flatten(),
                            emptyList(),
                            emptyMap(),
    ...
    class DokkaPsiParser(
        ...
        suspend fun parseClasslike(psi: PsiClass, parent: DRI): DClasslike = coroutineScope {
            with(psi) {
